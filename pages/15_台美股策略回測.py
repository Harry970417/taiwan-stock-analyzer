...
#
# This page is a RESULTS DISCLOSURE, not a sales page. In the default
# showcase mode (BACKTEST_DISPLAY_MODE=showcase, the deployment default),
# every number is read at runtime from the versioned, git-tracked release
# package in assets/backtest_release/v1/ (built by
# scripts/dev/build_release_assets.py from Phase 3/3.5 outputs) -- nothing
# here is hand-typed, no external API or network access is required, and
# the gitignored exports/ working directory is never touched. Showcase
# mode also runs a release-integrity gate (modules/release_validation.py:
# manifest presence, checksums, required columns/units, no secrets, no
# forbidden claims) before rendering anything; on failure the page shows
# only a one-line Chinese message, never a traceback or file path -- full
# technical detail goes to the application log instead.
#
# BACKTEST_DISPLAY_MODE=research (opt-in, NOT the deployment default) may
# instead read the complete exports/tw_us_backtest/ working directory for
# local development and audit workflows; it skips the release-manifest
# gate since exports/ has no manifest concept, but keeps the same
# fail-loud-on-missing-file behavior.
...
from modules.display_mode import get_display_mode, get_data_root, get_dataset_path, SHOWCASE
from modules.release_validation import is_release_valid

logger = logging.getLogger(__name__)

DISPLAY_MODE = get_display_mode()
DATA = get_data_root(ROOT, DISPLAY_MODE)
CHARTS = DATA / "charts"
...
if DISPLAY_MODE == SHOWCASE:
    # Structural guarantee (not just a comment): showcase mode must never
    # resolve to the gitignored exports/ working directory. get_data_root()'s
    # showcase branch cannot return an exports/ path by construction; this
    # assertion is a cheap, redundant, defense-in-depth check of that fact.
    data_parts = str(DATA).replace("\\", "/").split("/")
    assert "exports" not in data_parts, "showcase mode must not read exports/"

    @st.cache_data(show_spinner=False)
    def _cached_release_validation(release_dir_str: str, manifest_mtime: float):
        return is_release_valid(Path(release_dir_str))

    _manifest_path = DATA / "manifest.json"
    _manifest_mtime = _manifest_path.stat().st_mtime if _manifest_path.exists() else -1.0
    _valid, _check_results = _cached_release_validation(str(DATA), _manifest_mtime)
    if not _valid:
        for _r in _check_results:
            if not _r["passed"]:
                logger.error("Release validation failed: %s -- %s", _r["name"], _r["detail"])
        st.error("正式研究展示資料未通過完整性驗證，請重新建立 release assets。")
        st.stop()


def require_csv(dataset_key: str) -> pd.DataFrame:
    path = get_dataset_path(ROOT, dataset_key, DISPLAY_MODE)
    if not path.exists():
        logger.error("Missing required dataset file for dataset_key=%s: %s", dataset_key, path)
        if DISPLAY_MODE == SHOWCASE:
            st.error("正式研究展示資料未通過完整性驗證，請重新建立 release assets。")
        else:
            st.error(f"缺少必要資料檔案（research 模式）：{dataset_key}")
        st.stop()
...
# ── 讀取資料（showcase：版本化正式展示資產包；research：完整 exports/ 工作目錄）──
comparison   = require_csv("strategy_comparison")
kpi_head     = require_csv("core_metrics").iloc[0]
multi_seed   = require_csv("multi_seed_summary")
subperiod    = require_csv("subperiod_summary")
cost_stress  = require_csv("cost_stress_summary")
remove_year  = require_csv("remove_best_year_summary")
mdd_quant    = require_csv("drawdown_summary")
