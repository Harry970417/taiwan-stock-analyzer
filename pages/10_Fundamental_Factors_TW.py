# pages/10_Fundamental_Factors_TW.py
# 財報因子分析：四維評分（成長／品質／估值／現金流）+ 資料完整性檢查
...
with st.sidebar:
    ...
    <div style="font-size:0.9rem;font-weight:800;color:#E2E8F0;">📊 財報因子分析</div>
    <div style="font-size:0.7rem;color:#64748B;">基本面深度分析 + 風險評估</div>
...
if not run:
    ...
    for col, dim, desc in zip(
        ...
        ["營收年增率、EPS年增率、季增趨勢","ROE、毛利率、資本運用效率","本益比 vs 獲利能力","以營收趨勢作為現金流代理指標"]
    ):
    ...
    <h4>資料品質規則</h4>
    <p>• 所有比率自動判斷單位（小數 0-1 或百分比 0-100）並正規化<br>
    • 以台積電財報數值作為基準做合理性檢查：毛利率 0-90%、ROE -50 至 +100%<br>
    • 偵測到資料對應異常時，該項評分會暫停並標示<br>
    • 缺少的維度會降低信心分數，絕不虛構數值</p>
...
with col1:
    with st.spinner(f"載入 {ticker} 財報資料中..."):
...
cols_meta[0].markdown(f"""<div style="background:{'#FEF2F2' if has_issue else '#F0FDF4'};
    ...
    {'⚠ 偵測到資料對應異常' if has_issue else f'✅ 資料正常 · 完整度 {comp}% · {avail}/6 項因子'}
</div>""", unsafe_allow_html=True)

if has_issue:
    st.error("⚠ **資料對應異常**：" + "、".join(fund.get("sanity_warnings",[])))
    st.warning("基本面評分已暫停計算。原始數值可能有單位誤判（小數 vs 百分比），建議至公開資訊觀測站核對。")
...
with score_col:
    ...
    <div style="font-size:0.7rem;font-weight:700;color:#64748B;text-transform:uppercase;">基本面品質</div>
    ...
    <div style="font-size:0.7rem;color:#94A3B8;margin-top:0.3rem;">信心水準：{conf_s}%（{conf_l}）</div>
    </div>""", unsafe_allow_html=True)
...
kpi_with_interp(k1, "每股盈餘（元）",     eps,  "",  f"年增 {eps_yoy:+.1f}%" if eps_yoy else "無趨勢資料", "up" if (eps or 0)>0 else "down")
kpi_with_interp(k2, "股東權益報酬率",       roe,  "%", "優異 >25% / 平均 >15%",    "up" if (roe or 0)>15 else "flat")
kpi_with_interp(k3, "毛利率",  gm,   "%", "台積電約 53-55%",            "up" if (gm or 0)>40 else "flat")
kpi_with_interp(k4, "淨利率",    nm,   "%", "台積電約 38-42%",            "up" if (nm or 0)>15 else "flat")
kpi_with_interp(k5, "營收年增率",   yoy,  "%", "與去年同期比較",           "up" if (yoy or 0)>0 else "down")
kpi_with_interp(k6, "本益比",     pe,   "x", "合理區間 15-25 倍",             "up" if pe and pe<20 else "flat")
...
if fscore.get("notes") or fscore.get("alerts") or fscore.get("warnings"):
    ...
    with nf_col:
        st.markdown('<div style="font-size:0.7rem;font-weight:700;color:#16A34A;text-transform:uppercase;margin:0.5rem 0 0.4rem;">✦ 正向因子</div>', unsafe_allow_html=True)
    ...
    with al_col:
        st.markdown('<div style="font-size:0.7rem;font-weight:700;color:#DC2626;text-transform:uppercase;margin:0.5rem 0 0.4rem;">✦ 風險提示</div>', unsafe_allow_html=True)
...
if fund.get("eps_history"):
    ...
    if "yoy" in eps_df.columns:
        ...
        fig_eps.add_trace(go.Scatter(x=valid_yoy["date"],y=valid_yoy["yoy"],
            name="年增率 %",mode="lines+markers",line=dict(color="#F59E0B",width=2),yaxis="y2"))
        ...
        for inst_name, td in trend_data.items():
            ...
            <div style="background:#FFFFFF;border:1px solid #E2E8F0;border-left:4px solid {bc};
                ...
                <div style="display:flex;gap:1.5rem;margin-top:0.5rem;font-size:0.8rem;color:#475569;">
                    <span>近5日淨買賣：<b style="color:{'#16A34A' if td['net_5d']>0 else '#DC2626'}">{td['net_5d']:+,}</b></span>
                    <span>近20日淨買賣：<b style="color:{'#16A34A' if td['net_20d']>0 else '#DC2626'}">{td['net_20d']:+,}</b></span>
                    <span>近90日淨買賣：<b style="color:{'#16A34A' if td['net_90d']>0 else '#DC2626'}">{td['net_90d']:+,}</b></span>
                    <span>連續天數：<b>{td['consecutive']:+d}天</b></span>
                </div>
...
if show_risk and risk_data:
    ...
    r5.markdown(kpi_card("建議停損區間",
        ...
        f"約低於現價 {risk_data.get('sl_pct','?')}%"), unsafe_allow_html=True)
...
st.markdown(f"""<div class="research-box">
<h4>資料來源與完整性聲明</h4>
<p><b>資料來源：</b>{' · '.join(fund.get('data_sources',['N/A']))} <br>
<b>資料完整度：</b>{comp}%（6 項核心因子中可取得 {avail} 項）<br>
<b>比率正規化：</b>FinMind 小數比率（0-1）已自動轉換為百分比尺度（0-100）<br>
<b>合理性驗證：</b>所有指標均以台股實證校準區間檢查<br>
<b>對應異常：</b>{len(fund.get('sanity_warnings',[]))} 項指標被標示 — {'、'.join(fund.get('data_issues',['無'])) if fund.get('data_issues') else '無'}<br>
<b>缺失資料處理原則：</b>顯示為 N/A，絕不推估或替代</p>
</div>""", unsafe_allow_html=True)

st.markdown('<div style="text-align:center;font-size:0.72rem;color:#94A3B8;padding:1rem 0;">財報因子分析 · 僅供學術研究與作品集展示，不構成投資建議</div>', unsafe_allow_html=True)
